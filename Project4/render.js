...
function RenderGround(thickness, gridUnitSize)
{
    ...
    materialDiffuse = vec4(0.95, 0.9, 0.4, 1.0); //main coloring
...
function RenderObelisk(x, y, z, width, depth, height, rotY)
{
    mvMatrixStack.push(modelViewMatrix);

    lightAmbient = vec4(.4, .4, .4, 1.0);
    lightDiffuse = vec4(.8, .8, .8, 1.0);
    lightSpecular = vec4(.8, .8, .8, 1.0);

    materialAmbient = vec4(.8, .8, .8, 1.0);
    materialDiffuse = vec4(0.7, 0.5, 0.1, 1.0); //main coloring
    materialSpecular = vec4(1, 1, 1, 1.0);

    materialShininess = 50.0;
    sendLightingToGPU();
    gl.uniform1i(gl.getUniformLocation(program, "texture"), gl.TEXTURE[0]); //not working???
    transformations(x, y, z, 0, rotY, 0, width, height, depth);
    gl.drawArrays(gl.TRIANGLES, shapesInfo["obelisk"].start, shapesInfo["obelisk"].count);
    resetLighting();

    modelViewMatrix = mvMatrixStack.pop();
}

function RenderJar(x, y, z, rotX, rotY, rotZ, sca)
{
    mvMatrixStack.push(modelViewMatrix);
    ...
    materialAmbient = vec4(.8, .8, .8, 1.0);
    materialDiffuse = vec4(0.6, 0.3, 0.0, 1.0); //main coloring
    materialSpecular = vec4(1, 1, 1, 1.0);
    sendLightingToGPU();
...
function RenderSandstorm(x, y, z, rotY, sca)
{
    mvMatrixStack.push(modelViewMatrix);
    ...
    materialDiffuse = vec4(0.9, 0.9, 0.6, 1.0); //main coloring
    materialSpecular = vec4(0, 0, 0, 1.0);
    sendLightingToGPU();

    transformations(x, y, z, 0, rotY, 0, sca, sca, sca);
    gl.uniform1i(gl.getUniformLocation(program, "texture"), 0);
    gl.drawArrays(gl.POINTS, shapesInfo["sandstorm"].start, shapesInfo["sandstorm"].count);

    resetLighting();
    modelViewMatrix = mvMatrixStack.pop();
}

function RenderSarcophagus(x, y, z, rotX, rotY, rotZ, sca)
{
    mvMatrixStack.push(modelViewMatrix);
    ...
    materialAmbient = vec4(.8, .8, .8, 1.0);
    materialDiffuse = vec4(0.9, 0.6, 0.3, 1.0); //main coloring
    materialSpecular = vec4(1, 1, 1, 1.0);
    sendLightingToGPU();

    transformations(x, y, z, rotX, rotY, rotZ, sca, sca, sca);
    //gl.uniform1i(gl.getUniformLocation(program, "texture"), 0);
    gl.drawArrays(gl.TRIANGLE_FAN, shapesInfo["sarcophagus"].start, shapesInfo["sarcophagus"].count);
...
function RenderAnkh(x, y, z, rotX, rotY, rotZ, sca)
{
    mvMatrixStack.push(modelViewMatrix);
    ...
    transformations(x, y, z, rotX, rotY, rotZ, sca);
    GenerateAnkh();

    //RenderTorus(x, y, z, rotX, rotY, rotZ, sca);
    //RenderCylinder(1, 1, 0, 0, 0, 0, 1, 1);
    //RenderCylinders();
